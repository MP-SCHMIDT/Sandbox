...
void TheBoardGameAI::ComputeBoardScoreChk(BoardState *ioBoard) {
  ...
  bool isWinRed= true;
  bool isWinBlu= true;
  for (int k= 0; k < ioBoard->Pawns.nK; k++) {
    if (ioBoard->Pawns(k) < 0) isWinRed= false;
    if (ioBoard->Pawns(k) > 0) isWinBlu= false;
  }
  ...
  if (isWinRed) ioBoard->Score= +INT_MAX;
    ...
    // Count the material
    for (int k= 0; k < ioBoard->Pawns.nK; k++)
      if (ioBoard->Pawns(k) != 0) ioBoard->Score+= ioBoard->Pawns(k) * D.UI[ChkMaterial_____].I();
  }
}
